...
exports.generateKey = async function (password, options) {
    ...
    if (!password) {
        throw Boom.internal('Empty password');
    }

    if (!options ||
        typeof options !== 'object') {

        throw Boom.internal('Bad options');
    }

    const algorithm = exports.algorithms[options.algorithm];
    if (!algorithm) {
        throw Boom.internal('Unknown algorithm: ' + options.algorithm);
    }
    ...
    if (Buffer.isBuffer(password)) {
        if (password.length < algorithm.keyBits / 8) {
            throw Boom.internal('Key buffer (password) too small');
        }
        ...
        if (password.length < options.minPasswordlength) {
            throw Boom.internal('Password string too short (min ' + options.minPasswordlength + ' characters required)');
        }

        let salt = options.salt;
        if (!salt) {
            if (!options.saltBits) {
                throw Boom.internal('Missing salt or saltBits options');
            }
...
exports.seal = async function (object, password, options) {
    ...
    if (password.id) {
        if (!/^\w+$/.test(password.id)) {
            throw Boom.internal('Invalid password id');
        }
...
exports.unseal = async function (sealed, password, options) {
    ...
    if (parts.length !== 8) {
        throw Boom.internal('Incorrect number of sealed components');
    }
    ...
    if (macPrefix !== exports.macPrefix) {
        throw Boom.internal('Wrong mac prefix');
    }
    ...
    if (expiration) {
        if (!expiration.match(/^\d+$/)) {
            throw Boom.internal('Invalid expiration');
        }

        const exp = parseInt(expiration, 10);
        if (exp <= (now - (options.timestampSkewSec * 1000))) {
            throw Boom.internal('Expired seal');
        }
    }
    ...
    if (password instanceof Object &&
        ...
        if (!password) {
            throw Boom.internal('Cannot find password: ' + passwordId);
        }
    }
    password = internals.normalizePassword(password);
    ...
    if (!Crypto.timingSafeEqual(new Buffer(mac.digest), new Buffer(hmac))) {
        throw Boom.internal('Bad hmac value');
    }
    ...
    if (decryptOptions.iv instanceof Error) {
        throw Boom.wrap(decryptOptions.iv);
    }
    ...
    try {
        ...
        throw Boom.internal('Failed parsing sealed object JSON: ' + err.message);
    }
...
internals.stringify = function (object) {
    ...
    try {
        ...
        throw Boom.internal('Failed to stringify object: ' + err.message);
    }
};
