...
exports.generateKey = async function (password, options) {
    ...
    if (!password) {
        throw Boom('Empty password');
    }

    if (!options ||
        typeof options !== 'object') {

        throw Boom('Bad options');
    }

    const algorithm = exports.algorithms[options.algorithm];
    if (!algorithm) {
        throw Boom('Unknown algorithm: ' + options.algorithm);
    }
    ...
    if (Buffer.isBuffer(password)) {
        if (password.length < algorithm.keyBits / 8) {
            throw Boom('Key buffer (password) too small');
        }
        ...
        if (password.length < options.minPasswordlength) {
            throw Boom('Password string too short (min ' + options.minPasswordlength + ' characters required)');
        }

        let salt = options.salt;
        if (!salt) {
            if (!options.saltBits) {
                throw Boom('Missing salt or saltBits options');
            }
...
exports.seal = async function (object, password, options) {
    ...
    if (password.id) {
        if (!/^\w+$/.test(password.id)) {
            throw Boom('Invalid password id');
        }
...
exports.unseal = async function (sealed, password, options) {
    ...
    if (parts.length !== 8) {
        throw Boom('Incorrect number of sealed components');
    }
    ...
    if (expiration) {
        if (!expiration.match(/^\d+$/)) {
            throw Boom('Invalid expiration');
        }

        const exp = parseInt(expiration, 10);
        if (exp <= (now - (options.timestampSkewSec * 1000))) {
            throw Boom('Expired seal');
        }
    }
    ...
    if (password instanceof Object &&
        ...
        if (!password) {
            throw Boom('Cannot find password: ' + passwordId);
        }
    }
    password = internals.normalizePassword(password);
    ...
    try {
        ...
        throw Boom('Failed parsing sealed object JSON: ' + err.message);
    }
...
internals.stringify = function (object) {
    ...
    try {
        ...
        throw Boom('Failed to stringify object: ' + err.message);
    }
};
